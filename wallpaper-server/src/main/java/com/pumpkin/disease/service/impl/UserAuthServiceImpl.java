...
public class UserAuthServiceImpl extends ServiceImpl<UserAuthMapper, UserAuth> implements UserAuthService {
    ...
    public void updateUserPassword(PasswordRequest passwordRequest) {
        ...
        //endregion
        //region 检查两次密码是否一致
        AssertUtil.assertIsNotPass(!StrUtil.equals(passwordRequest.getNewPassword(), passwordRequest.getConfirmPassword()), USER_CONFIRM_PASSWORD_ERROR);
        //endregion
    ...
    public boolean checkUsername(String username) {
        //检查用户名是否存在
        return baseMapper.selectCount(new LambdaQueryWrapper<UserAuth>()
                .eq(UserAuth::getUsername, username)
                .eq(UserAuth::getIsDeleted, NOT_DELETED)) > 0;
    }

    @Override
    public boolean checkByOpenId(String openId, String source) {
        //检查用户名是否存在
        return baseMapper.selectCount(new LambdaQueryWrapper<UserAuth>()
                .eq(UserAuth::getUsername, openId)
                .eq(UserAuth::getLoginType, LoginTypeEnum.getValueByDesc(source))) > 0;
    }
}

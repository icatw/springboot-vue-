...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements MenuService {
    ...
    @Override
    public List<Menu> listMenuPerm() {

        //region 查询Redis缓存中是否含有对应数据
        List<Menu> menuPermResponseList = redisCache.getCacheList(PERM_MENU_LIST);
        //endregion

        if (AssertUtil.isListEmpty(menuPermResponseList)) {
            //region 查询菜单信息并进行数据封装
            menuPermResponseList = this.listAllMenu(true);
            //endregion
    ...
    private void clearRedisCache() {
        ...
        redisCache.deleteObject(CURRENT_ROLE_MENU_LIST + "*");
    }

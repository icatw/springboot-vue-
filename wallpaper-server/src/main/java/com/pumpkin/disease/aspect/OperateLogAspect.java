...
import com.pumpkin.disease.request.security.wallpaper.UserRegisterRequest;
...
public class OperateLogAspect {
    ...
    public void afterOperate(JoinPoint joinPoint, Object response) {
        ...
        User userInfo;
        //region 判断接口是否为前台用户注册
        if (SystemConstant.LogType.REGISTER_OPERATE.equals(log.operateType())) {
            UserRegisterRequest userRegisterRequest = (UserRegisterRequest) joinPoint.getArgs()[0];
            userInfo = userDetailsService.loadUserByUsername(userRegisterRequest.getUsername());
        } else {
            userInfo = UserUtil.getUserInfo();
        }
        AssertUtil.assertIsNotPass(AssertUtil.isNull(userInfo), HttpCodeEnum.USER_IDENTITY_LOAD_FAIL.getMessage());
        //endregion
        ...
        OperateLog normalOperateLog = OperateLog
                ...
                .requestParams(LogTypeEnum.UPLOAD_OPERATE.getTypeName().equals(log.operateType()) ? "{}" : JSON.toJSONString(joinPoint.getArgs()[0]))
